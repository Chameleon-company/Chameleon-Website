...
// Adding content statically to the firebase collections for resource pages
// these will be fetched through backend GET APIs and will be displayed on the resource page
const { initializeApp } = require('firebase/app');
const firebaseConfig = require('../../firebaseConfig');
const { getFirestore, doc, addDoc, getDoc, setDoc, updateDoc, deleteDoc, collection, getDocs } = require('firebase/firestore');
...
async function setResourceContent() {

    const iotStatsContent = {
    ...
    //Initializing collections for resource pages
    const collections = {
        IotStats: iotStatsContent,
        IotTech: iotTechContent,
        IotSecurity: iotSecurityContent,
        IotUpdates: iotUpdatesContent
    };
    //Adding content to their respective collections
    for (const [collectionName, contentData] of Object.entries(collections)) {
        const collectionRef = collection(db, collectionName);
        for (const [key, content] of Object.entries(contentData)) {
            try {
                await addDoc(collectionRef, { [key]: content });
            } catch (error) {
                console.error(`Error adding ${key} to ${collectionName} collection:`, error);
            }
        }
    }

};
//Make sure that initialization of collections happens only once even if server restarts multiple times
exports.InitializeResourceCollections = async () => {
    const initStatusRef = doc(db, "InitializationStatus", "resourceContent");
    const initStatusDoc = await getDoc(initStatusRef);

    if (!initStatusDoc.exists()) {
        await setResourceContent();
        await setDoc(initStatusRef, { initialized: true });
        console.log("Resource content initialized");
    } else {
        console.log("Resource content already initialized");
    }
}